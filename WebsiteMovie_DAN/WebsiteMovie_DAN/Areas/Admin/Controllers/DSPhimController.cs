...
namespace WebsiteMovie_DAN.Areas.Admin.Controllers
{
    public class DSPhimController : Controller
    {
        ...
        public ActionResult SuaPhim(int id, FormCollection collection, HttpPostedFileBase fileUpload)
        {
            ...
            {
                ViewData["Loi"] = "Tên phim không được để trống !";
            }
            else
                if (String.IsNullOrEmpty(thoiluong))
            {
                ViewData["Loi1"] = "Thời lượng không được để trống !";
            }
            else
            {
                if (fileUpload == null)
        ...
        //Xóa phim lẻ
        public ActionResult XoaPhimLe(int id)
        {
            DSPhimLe tk = data.DSPhimLes.SingleOrDefault(n => n.ID == id);
            if (tk == null)
            {
                Response.SubStatusCode = 404;
                return null;
            }
            data.DSPhimLes.DeleteOnSubmit(tk);
            data.SubmitChanges();
